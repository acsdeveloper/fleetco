...
function build_url($protocol, $host, $base_path, $url) {
  ...
  if ( !in_array(mb_strtolower($protocol), array("http://", "https://", "ftp://", "ftps://")) ) {
    ...
    if ( $url[0] !== '/' && (strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN' || ($url[0] !== '\\' && $url[1] !== ':')) ) {
      ...
      $rp = realpath($base_path);
	  if($rp == false)
	  {
		$rp = getabspath($base_path);
	  }
	  $ret .= $rp.'/';
    }
    $ret .= $url;
...
function dompdf_getimagesize($filename) {
  ...
  $eh = set_error_handler("record_warnings");
  list($width, $height, $type) = @getimagesize($filename);
  restore_error_handler( $eh );
  //@xlinesoft patch begin
  if( !$width && !$height && !in_array($type, array(IMAGETYPE_GIF, IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_BMP)) )
  {
	$imageData = xlinesoft_getFetchedImageData($filename);
	list($width, $height, $type) = array( $imageData['width'], $imageData['height'], null );
  }
  //@xlinesoft patch end
 ...
 * @return array 
 */
function xlinesoft_getFetchedImageData($filename)
{
	static $fetchedImageCache = array();
	
	if( isset( $fetchedImageCache[ $filename ] ) ) 
	  return $fetchedImageCache[ $filename ];
	
	$urlPathInfo = pathinfo( $filename );
	$parsedUrl = parse_url( $filename );

	$rawparams = explode('&', $parsedUrl['query']);
   ...
   $itype = SupposeImageType($file);
	...
	$fetchedImageCache[ $filename ] =  $data;
